...
export default class cart{
    async addToCart(product_id: number, customer_id: number, quantity: number): Promise<cartType>{
        ...
        try{
            ...
            if(quantityResult.rows[0].quantity < quantity) throw new quantityExceeds(`this quantity exceeds quantity supplied`);
            // insert into cart
            const query2 = `INSERT INTO cart VALUES(${customer_id}, ${product_id}, ${quantity}) returning *`;
            const cart = await conn.query(query2)
            if(cart.rows.length == 0) throw new Error('Can not insert this product into cart');
